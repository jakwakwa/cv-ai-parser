...
export const usePdfDownloader = () => {
	...
	const downloadPdf = async (element: HTMLElement | null, filename = "resume.pdf") => {
		if (!element) {
			toast({
				title: "Error",
		...
		try {
			if (typeof jsPDF === "undefined" || typeof html2canvas === "undefined") {
				toast({
					title: "Error",
					description: "PDF generation library not loaded. Please try again or use your browser's print to PDF function (Ctrl+P/Cmd+P).",
					variant: "destructive",
				})
				return
			}
