...
import type { FigmaAdaptationRequest, FigmaAdaptationResult } from '@/lib/ai-agents/figma-mcp-agent';
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!figmaLink) {
      ...
      } as AdaptFigmaResumeResponse, { status: 400 });
    }
    ...
    if (!figmaIds) {
      return NextResponse.json({
        ...
        details: 'The Figma link must be in the format: https://www.figma.com/file/FILE_KEY/... or https://www.figma.com/design/FILE_KEY/...'
      } as AdaptFigmaResumeResponse, { status: 400 });
    }
    ...
    if (!isAgentReady) {
      return NextResponse.json({
        ...
        details: 'The MCP server connection could not be established. Please check your configuration.'
      } as AdaptFigmaResumeResponse, { status: 503 });
    }
    ...
    const adaptationResult = await figmaMCPAgent.adaptDesignForResume(adaptationRequest);
    ...
    if (!adaptationResult.success) {
      ...
      } as AdaptFigmaResumeResponse, { status: 500 });
    }
    ...
    try {
      ...
      adaptationResult.adaptationLog.push(`Files saved: ${kebabName}.tsx, ${kebabName}.module.css`);
    } catch (writeError) {
...
export async function GET(): Promise<NextResponse> {
  ...
  } as AdaptFigmaResumeResponse, { status: 405 });
}