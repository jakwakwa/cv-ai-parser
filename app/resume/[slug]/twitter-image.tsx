...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
	try {
		...
		const skillsCount = typeof parsedData.skills && parsedData.skills !== null && Array.isArray(parsedData.skills) ? parsedData.skills.length : 0
					...
					<div
						...
						{experienceCount > 0 && (
							<div
								...
								<div style={{ fontSize: 28, fontWeight: "bold" }}>{experienceCount}</div>
								<div style={{ fontSize: 14, opacity: 0.9 }}>Experience{experienceCount !== 1 ? "s" : ""}</div>
							</div>
						)}
						...
						{skillsCount > 0 && (
							<div
								...
								<div style={{ fontSize: 28, fontWeight: "bold" }}>{skillsCount}</div>
								<div style={{ fontSize: 14, opacity: 0.9 }}>Skill{skillsCount !== 1 ? "s" : ""}</div>
							</div>
						)}
					</div>
				</div>
