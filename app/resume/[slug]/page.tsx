...
export default function ViewResumePage() {
		...
		const fetchResume = async () => {
			...
			try {
				...
				if (result.data?.parsedData?.metadata?.aiTailorCommentary) {
					setAiTailorCommentary(result.data.parsedData.metadata.aiTailorCommentary)
				} else {
					setAiTailorCommentary(null)
				}
			} catch (err: unknown) {
				setError((err as Error).message || "An error occurred while loading the resume.")
			} finally {
				setLoading(false)
			}
		}
	...
	const handleDownloadPdf = () => {
		if (resume) {
			downloadPdf(document.getElementById("resume-content") as HTMLElement, `${resume.parsedData.name?.replace(/ /g, "_") || "resume"}.pdf`)
		}
	}
	...
	if (loading) {
		return (
			<div className="loadingContainer">
				...
				<p className="loadingText">{viewMode === "edit" ? "Saving Changes..." : "Loading Resume..."}</p>
			</div>
		)
	}
	...
	if (!resume) {
		return (
			<div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6">
				...
				<p className="text-gray-700 mb-6 text-center">The resume you are looking for does not exist or is not public.</p>
				<Button onClick={() => router.push("/library")}>
					<ArrowLeft className="mr-2 h-4 w-4" /> Back to Library
				</Button>
			</div>
