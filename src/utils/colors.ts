...
function migrateOldResumeColorsToNew(oldColors: Record<string, string>): Record<string, string> {
	...
	for (const [oldKey, value] of Object.entries(oldColors)) {
		...
		if (newKey && newKey !== oldKey) {
			...
			console.warn(`Old resume color key '${oldKey}' found with no direct new mapping. Omitting from migrated resume colors.`)
	...
	if (!changed && Object.keys(oldColors).some(key => !key.startsWith("--resume-"))) {
