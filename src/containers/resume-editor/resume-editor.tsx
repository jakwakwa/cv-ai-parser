...
const ResumeEditor = ({ resumeData, onSave, onCancel, onCustomColorsChange }: ResumeEditorProps) => {
	...
	const handleContactChange = useCallback(
		<K extends keyof NonNullable<ParsedResumeSchema["contact"]>>(field: K, value: NonNullable<ParsedResumeSchema["contact"]>[K]) => {
	...
	const isSummaryValid = !editedData.summary || editedData.summary.length <= SUMMARY_MAX_CHARS
			...
			<div className={styles.wrapper}>
              ...
              >
                ...
                {showPreview ? 'Hide Preview' : 'Show Preview'}
							...
							<Button type="button" onClick={handleSave} className={styles.saveButton} disabled={!isSummaryValid}>
				...
				<div className={`${styles.grid} ${showPreview ? styles.gridTwoColumns : styles.gridOneColumn}`}>
					...
					<div className={styles.editorPanel}>
						...
						<SkillsEditorSection skills={editedData.skills || []} onSkillAdd={handleSkillAdd} onSkillRemove={handleSkillRemove} />

						<Card className={styles.card}>
							<SectionHeader title="Experience" onAdd={addExperience} />
							<CardContent className={styles.spaceY4}>
								{editedData.experience?.length === 0 && <p className="text-gray-500 text-sm">No experience added yet. Click "Add Experience" to start.</p>}
								{editedData.experience?.map((job, index) => (
									<ExperienceItem key={job.id} job={job} index={index} onChange={handleExperienceChange} onRemove={removeExperience} />
								))}
							</CardContent>
						</Card>
						...
						<Card className={styles.card}>
							...
							<CardContent className={styles.spaceY4}>
								{editedData.education?.length === 0 && <p className="text-gray-500 text-sm">No education added yet. Click "Add Education" to start.</p>}
								{editedData.education?.map((edu, index) => (
									<EducationItem key={edu.id} edu={edu} index={index} onChange={handleEducationChange} onRemove={removeEducation} />
								))}
							</CardContent>
						</Card>
						...
						<Card className={styles.card}>
							...
							<CardContent className={styles.spaceY4}>
								{editedData.certifications?.length === 0 && <p className="text-gray-500 text-sm">No certifications added yet. Click "Add Certification" to start.</p>}
								{editedData.certifications?.map((cert, index) => (
									<CertificationItem key={cert.id} cert={cert} index={index} onChange={handleCertificationChange} onRemove={removeCertification} />
								))}
							</CardContent>
						</Card>
					</div>
