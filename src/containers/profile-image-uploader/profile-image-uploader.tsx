...
const ProfileImageUploader = ({ currentImage, onImageChange, showPrompt = false, onSkip, onComplete }: ProfileImageUploaderProps) => {
		...
		return (
			<Card className={styles.card}>
				<CardHeader className={styles.cardHeader}>
					...
					<CardDescription className={styles.cardDescription}>Make your resume stand out with a professional photo.</CardDescription>
				</CardHeader>
				<CardContent className={styles.cardContent}>
					<button
						type="button"
						className={`${styles.uploadArea} ${dragActive ? styles.uploadAreaDragActive : previewImage ? styles.uploadAreaWithImage : styles.uploadAreaDefault}`}
						onDragEnter={handleDrag}
						onDragLeave={handleDrag}
						onDragOver={handleDrag}
						onDrop={handleDrop}
						onClick={handleButtonClick}>
						<input ref={fileInputRef} type="file" accept="image/jpeg,image/jpg,image/png,image/webp" onChange={handleFileSelect} className={styles.fileInput} />
						...
						{isLoading ? (
							...
							<div className={styles.previewContainer}>
								<div className={styles.imageWrapper}>
									<Image src={previewImage || "/placeholder.svg"} alt="Profile preview" className={styles.previewImage} width={400} height={400} />
									<button
										type="button"
										onClick={e => {
											e.stopPropagation()
	...
	return (
		<div className={styles.container}>
			<div className={styles.editorContainer}>
				<div className={styles.editorContent}>
					{previewImage && (
						<div className={styles.editorImageWrapper}>
							<Image src={previewImage || "/placeholder.svg"} alt="Profile" className={styles.editorImage} width={400} height={400} />
							<button type="button" onClick={handleRemoveImage} className={styles.editorRemoveButton}>
				...
				<button
					type="button"
					className={`${styles.editorUploadArea} ${dragActive ? styles.editorUploadAreaDragActive : styles.editorUploadAreaDefault}`}
					onDragEnter={handleDrag}
					onDragLeave={handleDrag}
					onDragOver={handleDrag}
					onDrop={handleDrop}>
