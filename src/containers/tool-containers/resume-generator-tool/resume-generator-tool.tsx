...
const ResumeGeneratorTool = ({ isLoading = false, isAuthenticated = false }: ResumeGeneratorToolProps) => {
	const {
		state,
		fileInputRef,
		handleFileChange,
	...
	const shouldShowLoading = isLoading || state.isProcessing || state.streamingProgress > 0 || state.streamingMessage !== ""

	if (shouldShowLoading) {
		return <LoadingState streamingMessage={state.streamingMessage || "Processing your resume..."} streamingProgress={state.streamingProgress} partialResumeData={state.partialResumeData} />
	}
