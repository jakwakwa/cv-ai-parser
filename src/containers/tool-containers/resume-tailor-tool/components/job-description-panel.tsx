    ...
    return (
        <div className={`${styles.panel} ${styles.disabledPanel}`}>
            <div className={styles.panelHeader}>
                ...
                <h2 className={styles.panelTitle}>Add a Job Description</h2>
            </div>
            ...
            {state.jobSpecMethod === "paste" ? (
                <div className={styles.textareaContainer}>
                    <Textarea className={styles.jobSpecTextarea} placeholder="Paste the job description here..." maxLength={4000} value={state.jobSpecText} onChange={e => onJobSpecTextChange(e.target.value)} />
                    <div className={styles.characterCount}>
                        <span className={state.jobSpecText.length > 3800 ? styles.characterWarning : ""}>{state.jobSpecText.length}/4000 characters</span>
                    </div>
                </div>
            ) : (
                <div className={styles.fileUploadContainer}>
                    <input type="file" ref={jobSpecFileInputRef} accept=".pdf" onChange={onJobSpecFileChange} className={styles.fileInput} />
                    <button type="button" onClick={() => jobSpecFileInputRef.current?.click()} className={styles.fileUploadButton}>
                        {state.jobSpecFile ? (
                            <>
                                <FileUpIcon size={20} />
            ...
            <div className={styles.instructionsSection}>
                ...
                />
                <div className={styles.characterCount}>
                    <span className={state.extraPrompt.length > 450 ? styles.characterWarning : ""}>{state.extraPrompt.length}/500 characters</span>
                </div>
            </div>

            {
                <div className={styles.actionSection}>
                    ...
                    <Button
                        ...
                        disabled={isLoading || !state.uploadedFile || (state.jobSpecMethod === "paste" ? !state.jobSpecText.trim() : !state.jobSpecFile)}
                        className={styles.createButton}
                        variant="primary"
                        size="lg">
                        <Upload size={20} />
                        Start Ai Tailoring Process
                    </Button>
                </div>
            }
        </div>
